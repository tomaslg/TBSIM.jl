using Distributions,SpecialFunctions,LinearAlgebra,Random,Interpolations
include("backtr.jl")
include("cova.jl")
include("dual.jl")
include("krige.jl")
...
function tbsim(
    simucoord=Float64[],datacoord=[],ydata=[],nrealiz=10,nx=200,ny=300,nz=1,##nargin=2,
    ...
    octant = 1,ndata = 4,nbdecimal=3,ntok = 5000, return_simucoords=false
...
#   backtr.jl           : back-transformation from Gaussian to original values
#   cova.jl             : compute covariance values
#   dual.jl             : compute dual kriging weights
#   krige.jl            : compute simple or ordinary kriging weights
#   picksupr.jl         : build template of super-blocks centered at the block containing the node to simulate
#   search.jl           : search the indices of the data located in the neighborhood
#   setdual.jl          : set up right hand side member for dual kriging
#   setrot.jl           : set up matrix for rotation and reduction of coordinates
#   superblk.jl         : set up super-block strategy
#   tbmain.jl           : main routine for random number generation along the lines
#   vdc.jl              : create equidistributed directions on the sphere (Van der Corput sequence)
#-----------------------------------------------------------------------------------------------------------
...
#-------------------------------------------------
  ...
  #--------------------------------------------
    ...
    for n = 1:m3
      ...
      # Coordinates of the points to simulate
      #--------------------------------------
      # println("ones(m4,1)*x0, y0 + index*dy ones(m4,1)*(z0 .+ nnnz*dz)=ones("
      # ,m4,",1)*",x0,", ",y0," + ",index,"*",dy," ",",ones(",m4,",1)*(",z0," .+ ",nnz,"*",dz,")");
    ...
    COORDS=sortslices(#=reshape(hcat(=#COORDS#=...)',(nx*ny*nz,3))=#,dims=1)
  end
  # println("F:size(simu)=",size(simu))
  if return_simucoords
    return Float64.(outputformat),COORDS
  else
    return Float64.(outputformat)#,datacoord,extreme_coord,ydata,simucoord
  end
  #--------------------------------------------------------------------------------------------
